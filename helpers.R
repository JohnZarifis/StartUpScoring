...
Bass.Default.Model <- function(M,P,Q,t1,t2,Inc){
  ...
  #print(Sales)
  Sales <- round(Sales)
  Sales.frame <- data.frame( 'Years'= t1:t2,'Sales'= Sales,'CumSales'= cumsum(Sales),'Income' = Sales*Inc,'Cost'= cost[cost$YEAR<=t2 & cost$YEAR>=t1,"COST"])
...
cost <- read_excel("~/Projects/StartUpScoring/Market.xlsx", sheet = 2 ,col_names = TRUE, na='na')
#View(cost)
